...
Reverse sync (Freshsales → hub): Freshsales → Admin → Workflows, create
a Deal workflow that fires on stage change and POSTs deal id + stage id
as JSON to this Worker's workers.dev URL at /api/sync/freshsales, with
header X-Freshsales-Sync-Key matching the FRESHSALES_SYNC_KEY secret.
That handler looks up the hub deal by its stored Freshsales deal id,
updates only its stage, and writes straight to D1 — it never calls
syncToCRM(), so a Freshsales-originated change can flow into the hub
